...
interface MigrationClientProps {
  initialMigrationJobs: MigrationJob[]
}

// ─── Page Component ──────────────────────────────────────────

export function MigrationClient({ initialMigrationJobs }: MigrationClientProps) {
  const [expandedCard, setExpandedCard] = useState<string | null>(null)
  const [memberSearch, setMemberSearch] = useState('')
  const [members, setMembers] = useState<MemberAssignment[]>([])
  const [rollbackConfirm, setRollbackConfirm] = useState<string | null>(null)
  const [migrationJobs, setMigrationJobs] = useState<MigrationJob[]>(initialMigrationJobs)
