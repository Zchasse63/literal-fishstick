  ...
  return (
    <div className="bg-white dark:bg-gray-950 rounded-xl border border-gray-200 dark:border-gray-800 shadow-lg p-3 text-xs">
      ...
      <p className="text-gray-600 dark:text-gray-400">
        {formatCurrency(value)} <span className="text-gray-400 dark:text-gray-500">({pct}%)</span>
      </p>
    </div>
  )
}
    ...
    fetch('/api/analytics/summary')
      ...
      .then((d) => {
        ...
        const metrics: KPIMetric[] = [
          { label: 'MRR', value: formatCurrency(raw.mrr?.value ?? 0), trend: raw.mrr?.trend ?? 0, href: '/revenue', iconName: 'DollarSign' },
          { label: 'ARPM', value: `$${(raw.arpm?.value ?? 0).toFixed(2)}`, trend: raw.arpm?.trend ?? 0, href: '/revenue', iconName: 'Activity' },
          { label: 'Active Members', value: String(raw.active_members?.value ?? 0), trend: raw.active_members?.trend ?? 0, href: '/members', iconName: 'Users' },
          { label: 'Churn Rate', value: `${(raw.monthly_churn_rate?.value ?? 0).toFixed(1)}%`, trend: raw.monthly_churn_rate?.trend ?? 0, href: '/members', iconName: 'Percent' },
          { label: 'Revenue MTD', value: formatCurrency(raw.revenue_mtd?.value ?? 0), trend: raw.revenue_mtd?.trend ?? 0, href: '/revenue', iconName: 'CreditCard' },
        ]
    ...
    <div className="min-h-screen bg-[#FAFAFA] dark:bg-[#0F0F11]">
      <div className="max-w-[1440px] mx-auto px-6 py-8 space-y-6">
        ...
        <motion.div {...fadeInUp}>
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Executive Overview</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          >
            <div className="mb-4">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Monthly revenue, last 12 months</p>
            </div>
              ...
              aiInsights.map((insight: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed mb-3">{insight.summary}</p>
                    <Link
