...
function TemplateCard({ template, delay }: { template: Template; delay: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-gray-950 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-5 flex flex-col gap-4 hover:border-indigo-200 hover:shadow-md transition-all group"
    >
      ...
      <div className="flex items-center justify-between mt-auto pt-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-medium text-gray-500 dark:text-gray-400">{template.steps} steps</span>
        </div>
        <Link
...
function AutomationCard({ automation, delay }: { automation: Automation; delay: number }) {
  ...
  return (
    <motion.div
      {...fadeInUp}
      transition={{ ...fadeInUp.transition, delay }}
      className="bg-white dark:bg-gray-950 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-5 hover:shadow-md transition-all"
    >
      {/* Top row: name + trigger badge + toggle */}
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-3 min-w-0">
          <div className={cn('h-10 w-10 rounded-xl flex items-center justify-center shrink-0', active ? 'bg-emerald-50' : 'bg-gray-100 dark:bg-gray-800')}>
            <Zap className={cn('h-5 w-5', active ? 'text-emerald-600' : 'text-gray-400 dark:text-gray-500')} />
          </div>
          <div className="min-w-0">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4 pb-4 border-b border-gray-100 dark:border-gray-800">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Enrolled</p>
          <div className="flex items-center gap-1.5 mt-1">
            <Users className="h-3.5 w-3.5 text-gray-400 dark:text-gray-500" />
            <p className="text-lg font-bold text-gray-900 dark:text-gray-100 tabular-nums">{automation.enrolled}</p>
          </div>
        </div>
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Completed</p>
          <div className="flex items-center gap-1.5 mt-1">
            <CheckCircle2 className="h-3.5 w-3.5 text-gray-400 dark:text-gray-500" />
            <p className="text-lg font-bold text-gray-900 dark:text-gray-100 tabular-nums">{automation.completed}</p>
          </div>
        </div>
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Conversion</p>
          <div className="flex items-center gap-1.5 mt-1">
            <Percent className="h-3.5 w-3.5 text-gray-400 dark:text-gray-500" />
            <p className="text-lg font-bold text-gray-900 dark:text-gray-100 tabular-nums">{automation.conversionRate}%</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-gray-500 dark:text-gray-400">Last triggered {automation.lastTriggered}</span>
        </div>
        <div className="flex items-center gap-2">
          <Link
            href={`/marketing/automations/${automation.id}`}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs font-semibold hover:bg-gray-200 transition-colors"
          >
...
function EmptyState() {
  return (
    <motion.div
      {...fadeInUp}
      className="bg-white dark:bg-gray-950 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-12 flex flex-col items-center justify-center text-center"
    >
    ...
    >
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Template Gallery</h2>
        </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Active Automations</h2>
        </div>
        {loading ? (
          <div className="bg-white dark:bg-gray-950 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-12 flex flex-col items-center justify-center text-center">
            <div className="h-8 w-8 rounded-full border-2 border-indigo-200 border-t-indigo-600 animate-spin mx-auto mb-3" />
            <p className="text-sm text-gray-500 dark:text-gray-400">Loading automations...</p>
          </div>
        ) : hasAutomations ? (
